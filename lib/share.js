...
/**
 * wifi-snap:// URL 스킴 링크 생성 (v0.11).
 * 데스크톱에 wifi-snap CLI를 v0.11+ 인스톨러로 깐 경우, 이 링크를 클릭하면
 * OS가 핸들러를 호출해서 자동으로 와이파이에 연결됨.
 *
 * 형식: wifi-snap://?wifi=BASE64
 */
export function createWifiSnapUrl({ ssid, password, security = 'WPA', label = null }) {
  const payload = { s: ssid, p: password || '', t: security };
  if (label) payload.l = label;
